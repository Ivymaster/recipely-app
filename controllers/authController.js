...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  //dobivanje korisnicko imena, i hasha, od API-a
  let odg = await axios.post(
    `${process.env.URL}/users/connect?apiKey=${process.env.API_KEY}`,
    params
  );
  if (!odg.data.username) {
    return next(new AppError('Korisnicko ime je zauzeto! Unesite drugo.', 400));
  }
  newUser.APIusername = odg.data.username;
  newUser.APIpassword = odg.data.hash;
